...
- `app/main.py` is the FastAPI entrypoint; `app/rag/` holds `api.py` (routes), `vector_store.py` (Redis/RediSearch), `chunking.py`, `embeddings.py`, `retrieval.py`, `llm.py`, `prompts.py`, `settings.py`.
...
- Redis env: set `REDIS_URL` (e.g., `redis://localhost:6379`), `REDIS_INDEX` (e.g., `rag:chunks`), optional `REDIS_PASSWORD`; set `OPENAI_API_KEY` to leave mock mode.
...
- Keep FastAPI routers thin; put Redis/RediSearch access in `vector_store.py` and business logic in helpers.
...
- Redis can be local or managed; for vectors we rely on RediSearch (Redis Stack). Do not send sensitive content to external services unless policy allows.
